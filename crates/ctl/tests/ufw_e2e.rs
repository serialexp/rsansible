//! End-to-end Phase 3 test: `ufw:` firewall management.
//!
//! Uses a stub `ufw` planted in /usr/local/bin so we can exercise the
//! full agent code path against an Alpine sshd container that has no
//! real ufw. The playbook handles the staging.
...
#[tokio::test]
#[ignore]
async fn ufw_drives_enable_rule_default_logging() -> Result<()> {
    ...
    let pb_path = examples_dir().join("ufw.yaml");
    ...
    // Final log should show the rule-allow + logging-high calls.
    let final_log = read_file(&container, "/tmp/rsansible-ufw-stub/log.final")?;
    assert!(
        final_log.contains("allow"),
        "expected `allow` in final log: {final_log:?}"
    );
    assert!(
        final_log.contains("logging high"),
        "expected `logging high` in final log: {final_log:?}"
    );
