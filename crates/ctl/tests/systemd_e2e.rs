//! End-to-end Phase 3 test: `systemd:` service management.
//!
//! Uses a stub `systemctl` planted in /usr/local/bin so we can exercise
//! the full agent code path against an Alpine sshd container that has
//! no real systemd. The playbook handles the staging.
...
#[tokio::test]
#[ignore]
async fn systemd_drives_start_enable_restart_mask() -> Result<()> {
    ...
    let pb_path = examples_dir().join("systemd.yaml");
    ...
    // daemon_reload-isolated log: daemon-reload must precede start.
    let dr_log = read_file(&container, "/tmp/rsansible-systemd-stub/log.dr")?;
    let dr_pos = dr_log.find("daemon-reload");
    let start_pos = dr_log.find("start foo.service");
    assert!(
        dr_pos.is_some() && start_pos.is_some() && dr_pos.unwrap() < start_pos.unwrap(),
        "daemon-reload must precede start in: {dr_log:?}"
    );

    // Final log should contain a `mask foo.service` invocation.
    let final_log = read_file(&container, "/tmp/rsansible-systemd-stub/log.final")?;
    assert!(
        final_log.contains("mask foo.service"),
        "mask invocation missing from: {final_log:?}"
    );
