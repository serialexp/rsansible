# rsansible — completed work

Historical record of work that has landed, split out of `TODO.md` to
keep the live file focused on what's still open. Items here retain
their original phase grouping for context.

---

## Phase 1 — Programming model (controller-only; no wire change)

The biggest architectural step out of v0. The orchestrator went from
"barrier loop over a flat task list" to a per-host data-flow graph:
registers feed conditions feed set_facts feed delegate_to feed
handlers. ~1500 LoC controller-side; lives in
`crates/ctl/src/orchestrator.rs` and `crates/ctl/src/exec_ctx.rs`.
...
~800 LoC. Mostly a precedence-walker and a small protocol change.
...
- [x] **Idempotency reporting sweep** — audited every agent module
  ...
  - `exec` / `shell`: `changed = (exit_code == 0)`. Diverges in
    ...
    the failure branch. Now that `ignore_errors:` has shipped, an
    ignored failure converts to `HostOutcome::Ok` at the per-host
    result boundary in the orchestrator — the register still records
    `.failed = true` + `.rc != 0`, but the play continues. The
    `changed = (rc == 0)` value isn't surfaced on the failed branch
    anyway because the converted outcome routes through the Ok arm
    without re-reading `done.changed`. If a future need for
    "changed-on-failure" appears (e.g. notifying handlers from an
    ignored failure), revisit in `modules/exec.rs:126` and the
    `enqueue_notifies` gate.
...
- [x] **`OpUri`** — HTTP client. `reqwest` 0.12 with `rustls-tls`
  (no OpenSSL) on the agent side, gated behind a new wire op kind 12.
  Subset of Ansible's `ansible.builtin.uri`: `url`, `method`
  (GET/POST/PUT/PATCH/DELETE/HEAD), `headers`, `body`, `body_format`
  (raw/json/form), `status_code` (single int or list), `timeout`
  (seconds → ms), `return_content`, `validate_certs`,
  `follow_redirects` (none/safe/all). Deferred to v2:
  `client_cert`/`client_key` (mTLS), `ca_path`, `dest`,
  `url_username`/`url_password` (caller can build the Authorization
  header themselves).

  Agent (`crates/agent/src/modules/uri.rs`, ~600 LoC incl. tests)
  emits a JSON envelope on stdout: `{status, url, headers, content?,
  content_length, content_type, json?, elapsed_ms, redirected}`. The
  envelope's `content` and `json` keys are omitted (not nulled) when
  not applicable. Status validation: if response code isn't in
  `status_codes`, exit_code=1 but the envelope still ships (so
  `register.status`/`register.content` populate even under
  `ignore_errors:`). Mutating verbs (POST/PUT/PATCH/DELETE) report
  `changed=1` on success; GET/HEAD always `changed=0` (Ansible's
  contract: idempotency is the server's responsibility). Pre-status
  errors (connect refused, DNS, TLS handshake, timeout) come back as
  TaskError with TIMEOUT or BAD_REQUEST codes — distinct from a
  500-with-body which goes through the envelope path.

  Redirect policy uses reqwest's `Policy::custom(...)` for the "safe"
  ...
  with a hand-written `Deserialize`: accepts method case-insensitively,
  `status_code` as int or list, `body` as YAML map/list (serialized to
  JSON at parse time when `body_format: json`), `headers` as a map,
  `timeout` as int/float seconds, `follow_redirects` as none/safe/all
  (case-insensitive) or a bool. Headers are stored in a `BTreeMap` so
  the wire ordering is deterministic. Validation refuses CR/LF/colon
  in header names (header-injection guard) and empty `status_codes`
  lists.

  Register-lifting (`lift_uri_envelope` helper in
  `crates/ctl/src/orchestrator.rs`) diverges from `OpStat`: Ansible's
  `uri:` contract exposes envelope keys at the **top level** of
  `register` (`register.status`, `register.content`,
  `register.headers`, `register.elapsed_ms`), not under
  `register.uri.*`. The envelope's parsed `json` body is swapped into
  `rv.json` so `register.json.<body-field>` resolves to the response
  body rather than the envelope itself. Canonical RegisterValue keys
  (`changed`/`rc`/`stdout`/`stderr`/`stdout_lines`/`took_ms`/`skipped`/
  `failed`/`results`) are shadow-protected against pathological
  response payloads.

  `become_::apply` adds `TaskOp::Uri(_)` to the non-argv pass-through
  list — `uri:` is HTTP from inside the agent process, no sudo
  wrapping.

  17 agent unit tests (axum 0.7 in-process server: method mapping,
  content-type detection, GET 200/404 envelope shape, status_code
  validation pass/fail, POST with auto Content-Type, POST with
  explicit Content-Type precedence, PUT form body, safe-redirect
  GET-follows + POST-doesn't-follow, none-redirect surfaces 3xx,
  connect refused → BAD_REQUEST, timeout → TIMEOUT, header
  normalization, elapsed_ms populated). 11 controller parse tests
  (defaults, method case-folding, status_code int/list, JSON-body
  auto-serialize, headers-non-map rejected, follow_redirects
  validation, timeout-as-float, missing-url rejected, to_wire_op
  round-trip). 2 register-lift unit tests (top-level key promotion +
  json-body swap + canonical-key shadow-protection; no-op when
  rv.json absent). E2E (`crates/ctl/tests/uri_e2e.rs`) spins up
  `busybox httpd` inside the container and verifies GET 200 lifts
  `register.json.foo`/`register.content`, status_code mismatch +
  `ignore_errors` exposes `register.failed && register.status==200`,
  and HEAD reports `changed=false`.

- [x] **`.j2` body precompile at validate time.** `check_op`'s
  `TaskOp::Template` arm already compiled `dest`; now it also compiles
  the loaded `t.body` if Some, so a Jinja syntax error inside a
  template file surfaces from `template::precompile_all` rather than
  at first task dispatch on a host. Error message includes the `src:`
  path so the user knows which `.j2` to fix. 2 unit tests construct a
  Playbook, inject body bytes by hand (the field is `#[serde(skip)]`),
  and verify the bad case fails with the src in the message + the
  clean case passes. `OpTemplate` itself and the custom filter set
  (`b64encode` / `b64decode` / `from_json` / `to_json` /
  `regex_replace` / `mandatory`) already shipped in Phase 2b/2c.
...
- [x] **`--limit` flag** — restricts the run to a host pattern.
  ...
  bail.
- [x] **`ignore_errors:`** — task-level opt-out from play-halting on
  failure. Parsed on `Task` as `Option<bool>` (`task_op.rs`,
  `METADATA_KEYS` extended, hard-fails on non-bool). Gate lives in
  the orchestrator at the per-host result boundary in
  `run_task_on_one_host`: a `maybe_ignore_failure` helper converts
  `HostTaskOutcome::Failed { reason }` → `Ok` for tasks where
  `ignore_errors: true` resolves at dispatch. Applied at both return
  sites (single-exec and loop-aggregate). The register is recorded
  with `.failed=true` + the captured `rc` *before* the conversion, so
  follow-up `when: prev.failed` / `assert: that: prev.failed` works
  as in Ansible. Handlers don't fire on ignored failures —
  `enqueue_notifies` is in the inner Ok-match arm before the
  boundary, matching Ansible's "no notify on ignored failure"
  semantics. Both `per_task` and `per_play` dispatch strategies
  inherit the conversion: per_task sees Ok at
  `apply_per_host_result`, per_play sees Ok at the
  `HostOutcome::Ok`-match line. 5 unit tests covering the rule
  table (ignored failure → Ok; non-ignored failure unchanged;
  Skipped/Ok/Changed pass-through) + 4 parser tests + a 2-scenario
  ignored e2e (`tests/ignore_errors_e2e.rs`): one playbook with
  `shell: "exit 7"` + `ignore_errors: true` + `register:` + a
  follow-up `assert: that: ["failing.failed", "failing.rc == 7"]` +
  a post-marker task (all three markers exist, host Ok,
